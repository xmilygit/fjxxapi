...
router.get('/xls2json', async (ctx, next) => {
    ...
    // })
    try {
        let data = await xls2json2("../public/upload/2020-08-17复审合格导出.xlsx", "桂林小学报名数据")
        ctx.body = data;
    } catch (err) {
...
router.post("/getnotice", async (ctx, next) => {
    ...
    try {
        let notice = await nsn.findOneByQuery({ "openid": wxopenid })
        ctx.body = { "error": false, "result": notice }
    } catch (err) {
...
router.post('/noticereaded', async (ctx, next) => {
    let wxopenid = ctx.request.body.wxopenid
    try {
        let notice = await nsn.updateone({ 'openid': wxopenid }, { $set: { "readed": 1 } })
        ctx.body = { "error": false, "result": notice }
    } catch (err) {
        ctx.body = { 'error': true, 'message': err.message }
    }
